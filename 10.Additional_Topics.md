...
Step 0: x₀ = clean image
Step 1: x₁ = x₀ + noise₁
Step 2: x₂ = x₁ + noise₂
...
Step T: xₜ = pure noise
...
Model learns: Given noisy image xₜ,
...
```
Forward process:
q(xₜ | xₜ₋₁) = 𝒩(√(1-βₜ) xₜ₋₁, βₜI)

Where:
βₜ = noise schedule (increases over time)
𝒩 = Gaussian distribution

Reverse process:
p(xₜ₋₁ | xₜ) = 𝒩(μθ(xₜ, t), Σθ(xₜ, t))

Model predicts μθ (mean) to denoise
```
...
guidance_scale = 7.5 (typical)
  Higher → Closer to prompt, less diverse
  Lower → More diverse, less adherent
...
Multimodal:
Text + Image → Multimodal Model → Text/Image Output
```
...
Image Embedding (512 dims)
     ↓
Similarity ← Compare → Text Embedding (512 dims)
                            ↑
...
Image → Vision Encoder → Visual Tokens
                              ↓
Text Prompt + Visual Tokens → LLM → Response
...
Text: "A cat"  → [0.1, 0.3, -0.2, ...]
Image: 🐱      → [0.12, 0.28, -0.18, ...]
Audio: "meow"  → [0.11, 0.31, -0.19, ...]

All close in embedding space!

Applications:
- Cross-modal search (text → find image)
...
RAD (multi-turn):
User: "What is Python?"
  ↓ Retrieve docs + conversation history
Assistant: "Python is a programming language..."

User: "When was it created?"
  ↓ Retrieve docs (understands "it" = Python) + history
...
└──────────┬──────────┘
           ↓
┌──────────────────────┐
│ Context Resolution   │ (Resolve "it", "he", "that")
└──────────┬───────────┘
           ↓
┌──────────────────────┐
│ Query Reformulation  │ (Expand query with context)
└──────────┬───────────┘
           ↓
┌──────────────────────┐
...
Example:
640×480 RGB image = 640 × 480 × 3 = 921,600 values
```
...
1. Molecular Property Prediction:
   Molecule → GNN → Properties (toxicity, solubility)

2. De Novo Drug Design:
   Target → Generative Model → Novel molecules

3. Protein Folding:
   Amino acid sequence → AlphaFold → 3D structure
```
...
Input → Router → Select Experts → Combine Outputs
...
Benefits:
✓ Higher capacity
✓ Lower inference cost (activate subset)
✓ Specialization
```
...
- **[03: Generative AI and LLMs](03.Generative_AI_and_LLM.md)** - Text generation models
- **[04: Transformer Architecture](04.Transformer_Architecture.md)** - Foundation for many models here
- **[06: Model Customization](06.Model_Customization.md)** - RAD builds on RAG
- **[07: Model Training](07.Model_Training.md)** - Training techniques for all models
- **[08: NVIDIA Ecosystem](08.NVIDIA_Ecosystem.md)** - Tools for deploying these models
