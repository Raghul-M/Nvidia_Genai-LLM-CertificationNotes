...
- Register file  (Registers hold temporary values for immediate computation.)
- Warp schedulers (Warp scheduler is a hardware unit inside an SM that selects and issues instructions from ready warps to execution units.)
...
- Tensor cores = 🧠 AI specialist workers
...
#### Note:
```
CUDA cores are general-purpose arithmetic units, while tensor cores are specialized units optimized for 
high-throughput matrix operations used in deep learning.
```
...
```
                            ...
                            L2 Cache (shared across GPU)
                                      ↓
                             L1 Cache (inside each SM)
                                      ↓
                   Shared Memory (inside each SM, programmable)
                                      ↓
                            Registers (inside each thread)
                                      ↓
                        CUDA Cores / Tensor Cores (compute units)
```
...
Example workflow:
1. Request 1GB for layer 1 → allocate → use → free
2. Request 2GB for layer 2 → allocate → use → free
3. Request 1GB for layer 3 → allocate → use → free
⏱️ Total overhead: 3 allocation + 3 deallocation calls

Bottleneck:
CPU ←→ GPU (limited throughput)
...
⏱️ Total overhead: 1 allocation call
...
Pros:
✓ Much faster allocation (50-100x)
...
✗ Memory not immediately returned to OS
...
- TF32 precision
- Structural sparsity
- Multi-Instance GPU (MIG) [Multi-Instance GPU (MIG) is a feature that partitions a single physical GPU into multiple independent hardware-isolated GPU instances.]
...
- L40S GPU
- 4th gen Tensor Cores
- 48GB GDDR6 (GDDR6 is a high-bandwidth graphics memory used in GPUs to store data and support parallel processing workloads such as gaming and AI inference. NVIDIA GeForce RTX 3080 uses GDDR6 / GDDR6X )
- DLSS 3
- Ray tracing
```
